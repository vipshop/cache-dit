...
_DEFAULT_MODEL_SOURCE = os.getenv(
...
def test_ray_wrapper_defers_cache_hooks_to_workers(monkeypatch: pytest.MonkeyPatch) -> None:
  transformer = _toy_transformer()
  parallelism_config = _toy_parallel_config()
  captured: dict[str, object] = {}

  def fail_main_process_cache_apply(*args, **kwargs):
  ...
  monkeypatch.setattr(
    ...
    fail_main_process_cache_apply,
  )
  monkeypatch.setattr(
    "cache_dit.ray.enable_ray_parallelism",
    fake_enable_ray_parallelism,
  )

  returned = cache_dit.enable_cache(
    transformer,
  ...
  assert isinstance(cache_context_kwargs["cache_config"], DBCacheConfig)
  assert captured["quantize_config"] is None


def test_ray_wrapper_defers_quantize_to_workers(monkeypatch: pytest.MonkeyPatch) -> None:
  transformer = _toy_transformer()
  parallelism_config = _toy_parallel_config()
  quantize_config = QuantizeConfig(quant_type="float8_per_row")
  captured: dict[str, object] = {}

  def fail_main_process_quantize(*args, **kwargs):
  ...
  assert captured["quantize_config"] is not quantize_config
