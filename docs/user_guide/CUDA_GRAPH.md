...
- <span style="color:#c77dff;">RuntimeError: accessing tensor output of CUDAGraphs that has been overwritten</span>: Why it happens? (1) A captured graph output is referenced after a later replay has already overwritten the same output buffer. (2) This often appears when CUDA Graph is combined with regional compile (`compile_repeated_blocks`) in transformer loops. Typical message:
    ```text
    RuntimeError: Error: accessing tensor output of CUDAGraphs that has been overwritten by a subsequent run.
    ```
  How to fix: (Quick check for Cache-DiT CLI, if you see this error in the logs, make sure to disable regional compile and use full-module compile for transformer by adding the flag: Use `--compile --cuda-graph --no-regional-compile`.)
...
- <span style="color:#c77dff;">CUDA Graph enabled but little/no speedup</span>. Possible reasons:
