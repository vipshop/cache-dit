...
def _svdq_quantize_w4a4_act_fuse_lora(
    ...
    if fp4:
        if channels % 16 != 0:
            raise ValueError(
                f"Expected channels divisible by 16 for FP4 quantization, got {channels}."
            )
        oscales = torch.empty(
            channels // 16, batch_size_pad, dtype=torch.float8_e4m3fn, device=input.device
        )
    else:
        if channels % 64 != 0:
            raise ValueError(
                f"Expected channels divisible by 64 for INT4 quantization, got {channels}."
            )
        oscales = torch.empty(
...
def _svdq_quantize_w4a4_wgt(input: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    output = torch.empty(
        (input.shape[0], input.shape[1] // 2), dtype=torch.int8, device=input.device
    )
    oscales = torch.empty(
        (input.shape[1] // 64, input.shape[0]), dtype=input.dtype, device=input.device
    )
