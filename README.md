<div align="center">
  ...
  <p align="center">
    DeepCache is for UNet, but DiT has no U-shape so it can't use it. Most DiT cache speedups are complex and not training-free—need to train extra modules, hard to apply. DBCache   fixes this: builds on FBCache to make a UNet-style, Training-free Cache Acceleration for DiT.
  </p>
</div>
...
The codebase of [DBCache](./src/cache_dit/) was adapted from [FBCache](https://github.com/chengzeyi/ParaAttention/tree/main/src/para_attn/first_block_cache) at [ParaAttention](https://github.com/chengzeyi/ParaAttention). Many thanks! **DBCache** is still in its early dev stages and now provides some docs and benchmarks for reference.
